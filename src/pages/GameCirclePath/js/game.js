...
playGame.prototype = {
  ...
  update() {
    const distanceFromTarget = this.balls[this.rotatingBall].position.distance(this.targetArray[1].position);
  ...
  changeBall() {
    ...
    if (distanceFromTarget < 20) {
      this.rotatingDirection = game.rnd.between(0, 1);
      const detroyTween = game.add.tween(this.targetArray[0]).to(
        {
          alpha: 0,
        },
        500,
  ...
  addTarget() {
    ...
    const text = game.add.text(0, 0, this.steps.toString(), style);
    text.anchor.set(0.5);
  ...
  gameOver() {
    ...
    this.arm.destroy();
    const gameOverTween = game.add.tween(this.balls[1 - this.rotatingBall]).to(
      {
        alpha: 0,
      },
      1000,
